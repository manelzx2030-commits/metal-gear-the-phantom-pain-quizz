/* =====================================================================
   ...
   - [BUG CORRIGIDO — MAPA] O mapa foi refeito do zero. A causa real
     das "letras exageradas" e do mapa "esquisito" no PC era o SVG
     desenhando o nome de cada local como <text> sem font-size, dentro
     de um viewBox pequeno (100x90) — o texto nunca escalava certo com
     a tela. Agora o SVG só desenha o terreno; cada localização é um
     pino HTML (tamanho fixo, fonte normal) posicionado por
     porcentagem sobre o terreno, com tooltip e sincronizado com a
     legenda. Ver buildMapSvg / buildMapMarkers / renderMap /
     setupMapInteractions e as classes .map-canvas / .map-pin em
     gear.css.
   - [BUG CORRIGIDO — LAYOUT] .idroid-content-inner (max-width) já
     existia no CSS mas nenhum elemento do HTML usava essa classe, ou
     seja, o limite de largura nunca era aplicado de verdade em telas
     grandes. Corrigido no index.html.
   ...
   - [CONTEÚDO] Player de fitas cassete via YouTube: cada fita abre um
     ...
     youtubeId for preenchido no CONFIG). O HTML do modal (#ytOverlay
     e afins) estava ausente do index.html e foi adicionado — sem ele
     o app travava ao inicializar o player, antes mesmo de configurar
     a navegação e o mapa.
   - [CONTEÚDO] Nova fita "Here's to You (Nicola and Bart)", marcada
     como a mais dramática, desbloqueada só após a missão final.
   - [CONTEÚDO] Busca/filtro na aba de Fitas Cassete (o campo
     #cassetteFilter também estava ausente do HTML e foi adicionado).
   - [CONTEÚDO] "Fita em destaque do dia" na tela inicial (o
     container #tapeOfDay também estava ausente do HTML e foi
     adicionado).
...
/* ---------------------------------------------------------------
   MAPA
   [REFEITO DO ZERO] O SVG agora desenha SÓ o terreno (grade + relevo),
   sem nenhum texto — era o texto sem font-size dentro de um viewBox
   pequeno (100x90) que aparecia gigante e distorcido em qualquer
   monitor. Cada localização vira um pino HTML de verdade
   (.map-pin), posicionado por porcentagem sobre o terreno, com
   tooltip ao passar o mouse/tocar e sincronizado com a legenda —
   clicar em um pino destaca o item da legenda (e vice-versa). Como
   tudo escala com % + aspect-ratio (ver gear.css), o mapa se
   redimensiona corretamente tanto no PC quanto no Android.
   --------------------------------------------------------------- */
...
/* Constrói os pinos em HTML. As coordenadas de cada local (loc.x,
   loc.y) já seguem o mesmo sistema 0-100 / 0-90 do viewBox do SVG,
   então left = x% e top = (y / 90 * 100)% caem exatamente no ponto
   certo do terreno, em qualquer tamanho de tela. */
function buildMapMarkers(data) {
  return data.locations.map((loc, i) => {
    const topPct = (loc.y / 90 * 100).toFixed(3);
    let edgeClass = '';
    if (loc.x <= 20) edgeClass = 'tip-right';
    else if (loc.x >= 80) edgeClass = 'tip-left';
    if (loc.y <= 14) edgeClass += (edgeClass ? ' ' : '') + 'tip-below';

    return `<button type="button" class="map-pin ${edgeClass}" style="left:${loc.x}%; top:${topPct}%;"
        data-index="${i}" aria-label="${loc.name}">
      <span class="map-pin-ring" aria-hidden="true"></span>
      <span class="map-pin-dot" aria-hidden="true">${i + 1}</span>
      <span class="map-pin-tip" role="tooltip">${loc.name}</span>
    </button>`;
  }).join('');
}
...
function renderMap(region) {
  ...
  document.getElementById('mapMarkers').innerHTML = buildMapMarkers(data);
  document.getElementById('mapRegionCaption').textContent = `Região selecionada: ${data.label}`;
  document.getElementById('mapLegend').innerHTML = data.locations.map((loc, i) =>
    `<div class="map-legend-item" data-index="${i}" tabindex="0" role="button" aria-label="Destacar ${loc.name} no mapa">
...
/* Destaca em conjunto o pino e o item de legenda correspondentes —
   é o que torna o mapa "interativo" nos dois sentidos: clicar no
   pino realça a legenda (e rola até ela), clicar na legenda realça
   o pino no mapa. */
function setActiveMapLocation(index) {
  document.querySelectorAll('.map-pin').forEach(pin => {
    pin.classList.toggle('active', Number(pin.dataset.index) === index);
  });
...
function setupMapInteractions() {
  ...
  panel.addEventListener('click', e => {
    ...
    if (pin) {
      ...
      setActiveMapLocation(index);
      const legendItem = document.querySelector(`.map-legend-item[data-index="${index}"]`);
      if (legendItem) legendItem.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      return;
    }
    const legendItem = e.target.closest('.map-legend-item');
    if (legendItem) setActiveMapLocation(Number(legendItem.dataset.index));
  });

  // Acessibilidade: permite selecionar um item da legenda via teclado.
  panel.addEventListener('keydown', e => {
    if (e.key !== 'Enter' && e.key !== ' ') return;
    const legendItem = e.target.closest('.map-legend-item');
    if (!legendItem) return;
    e.preventDefault();
    setActiveMapLocation(Number(legendItem.dataset.index));
  });
}
