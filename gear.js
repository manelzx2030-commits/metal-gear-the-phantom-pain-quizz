...
const CONFIG = {
  ...
  // (Os IDs não vêm pré-preenchidos porque variam e o vídeo "certo"
  // pode mudar; assim você garante que o vídeo embutido é sempre o
  // que você escolheu.)
  //
...
function buildMapMarkersHtml(region) {
  ...
  return data.locations.map((loc, i) => {
    ...
    let side = '';
    if (loc.x >= 66) side = 'tip-left';
    else if (loc.x <= 18) side = 'tip-right';
    return `<button type="button" class="map-pin ${side}" data-idx="${i}" style="left:${loc.x}%; top:${topPct}%;">
      ...
      <span class="visually-hidden">${loc.name}</span>
    </button>`;
  }).join('');
}
...
function renderMap(region) {
  ...
  const data = CONFIG.mapRegions[region];
  document.getElementById('mapStage').innerHTML = buildMapBackgroundSvg(region);
  const markersEl = document.getElementById('mapMarkers');
  if (markersEl) markersEl.innerHTML = buildMapMarkersHtml(region);
  ...
  document.getElementById('mapLegend').innerHTML = data.locations.map((loc, i) =>
    `<div class="map-legend-item" data-idx="${i}" tabindex="0">
...
/* Liga o marcador numerado no mapa com o card correspondente na
   legenda (tocar/clicar em um destaca e rola até o outro). Ligado uma
   única vez, pois o mapa e a legenda são recriados a cada renderMap(). */
function setupMapInteractions() {
  ...
  const highlight = idx => {
    canvas.querySelectorAll('.map-pin').forEach(p => p.classList.toggle('active', p.dataset.idx === idx));
    legend.querySelectorAll('.map-legend-item').forEach(l => l.classList.toggle('active', l.dataset.idx === idx));
  };

  canvas.addEventListener('click', e => {
    const pin = e.target.closest('.map-pin');
    if (!pin) return;
    highlight(pin.dataset.idx);
    const item = legend.querySelector(`.map-legend-item[data-idx="${pin.dataset.idx}"]`);
